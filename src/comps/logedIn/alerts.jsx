...
export const Alerts = () => {
  ...
  const imgChosen = (imgID) => {
    const imgInfo = MDI.find((img) => {
      console.log("Alerts: img._id", img._id);
      return img._id === imgID;
    });
    console.log("Alerts: imgInfo", imgInfo);

    setImgSelect((prev) => [...prev, imgInfo.view]);
  };
  console.log("Alerts: global", imgSelect);
  console.log("Alerts: MDI", MDI);
          ...
          <div className="assetsPanel">
            ...
            <div className="imageSelect">
              <select name="" id="" onChange={(e) => imgChosen(e.target.value)}>
                ...
                {MDI.length > 0 &&
                  MDI.map((image, i) => (
                    <option key={i} id={image._id} value={image._id}>
                      {image.name}
                    </option>
                  ))}
              </select>
