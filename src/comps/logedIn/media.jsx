...
export const Media = () => {
  ...
  const imgFetch = async (e) => {
    (imgData.view = preview), (imgData.name = image.name);
    ...
    <section className="pSection">
      ...
      <article className="pArticle mediaArticle">
        <div className="mediaSwitch">
          <button className="switchBox" onClick={() => tabHandler("images")}>
            Images
          </button>
          <button className="switchBox" onClick={() => tabHandler("sounds")}>
            Sounds
          </button>
          <button className="switchBox" onClick={() => tabHandler("videos")}>
            Videos
          </button>
          {preview ? (
                ...
                MDI[0].map((image, i) => {
                  return (
                    <div className="imgCard" id={image._id} key={i}>
                      ...
                      <div
                        className="deleteBtn"
                        onClick={() => deleteHandler(this.image)}
                      >
                        x
                      </div>
            ...
            <div>
              <div className="mediaBox">
                <form className="mediaSettings">
                  <label htmlFor="soundSwitch">On/Off</label>
                  <br />
                  <input
                    type="checkbox"
                    name="SoundVolumeSwitch"
                    id="soundSwitch"
                  />
                  <br />
